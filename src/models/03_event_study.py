...
referencia k = -1.  Excluye always-treated (sin pre-período) y
switchers left-censored (first_treat_t=0, sin pre-período).
...
def build_event_dummies(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    # Exclude left-censored switchers (first_treat_t == 0): they have no
    # pre-treatment observations so all their dummies are post-treatment only.
    # They don't contaminate pre-trends but inflate post estimates without
    # a proper counterfactual anchor.
    g0_mask = (df["cohort_type"] == "switcher") & (df["first_treat_t"] == 0)
